...
EXPECTED = (
    ...
    b'"max_score":65.762751,"quality":"pass"}}\n'
)
...
def test_outlier_batch_optional_keys_may_be_omitted(tmp_path):
    ...
    assert result.stdout == (
        b'{"results":[[false,0.67449,2.0],[false,0.0,3.0],'
        b'[false,0.67449,4.0]],'
...
def test_outlier_batch_threshold_only_may_be_given(tmp_path):
    document = {"depths": [1, 2, 3, 100], "threshold": 0.5}
    request = write_request(tmp_path / "request.json", document)
    result = run_cli(["outlier-batch", request], cwd=tmp_path)
    assert result.returncode == 0
    assert b'"outlier_count":2' in result.stdout
    assert result.stdout.endswith(b'"quality":"fail"}}\n')
...
def test_outlier_batch_key_order_enforced(tmp_path):
    raw = (
        b'{"max_outlier_ratio":0.5,"depths":[1,2,3,100],'
        b'"threshold":2.0}'
    )
...
def test_outlier_batch_threshold_after_ratio_rejected(tmp_path):
    raw = b'{"depths":[1,2,3,100],"max_outlier_ratio":0.5,"threshold":2.0}'
    request = write_request(tmp_path / "request.json", raw=raw)
    result = run_cli(["outlier-batch", request], cwd=tmp_path)
    assert result.returncode == 1
    assert result.stderr == (
...
def test_outlier_batch_duplicate_keys_rejected(tmp_path):
    raw = (
        b'{"depths":[1,2,3,100],"depths":[1,2,3,100],'
        b'"threshold":2.0}'
    )
    request = write_request(tmp_path / "request.json", raw=raw)
    result = run_cli(["outlier-batch", request], cwd=tmp_path)
...
def test_outlier_batch_domain_error_reported(tmp_path):
    document = dict(REQUEST, max_outlier_ratio=2.0)
    request = write_request(tmp_path / "request.json", document)
    result = run_cli(["outlier-batch", request], cwd=tmp_path)
    assert result.returncode == 1
    assert result.stdout == b""
...
def test_outlier_batch_detect_error_reported(tmp_path):
    ...
    result = run_cli(["outlier-batch", request], cwd=tmp_path)
    assert result.returncode == 1
    assert result.stderr == b"ERROR ValueError: depths must have at least 3 elements\n"


def test_outlier_batch_missing_argument_is_parse_error(tmp_path):
    result = run_cli(["outlier-batch"], cwd=tmp_path)
