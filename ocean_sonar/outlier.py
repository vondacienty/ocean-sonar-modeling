...
def batch(depths, threshold=3.5, max_outlier_ratio=0.1) -> bytes:
    ...
    must be a finite non-bool int/float lying in ``[0, 1]`` (finiteness is
    checked only for a float, but an int must still be ``0`` or ``1``). A
    ...
    ``k`` the number of flagged items.

    The returned document has top-level keys exactly in the order
    ``results, summary``: ``results`` is an array in the order of ``R``
    of ``[is_outlier, score, depth]`` arrays with the values taken
    unchanged from each ``R[i]``; ``summary`` has keys exactly in the
    order ``count, outlier_count, outlier_ratio, max_score, quality``
    where ``count`` and ``outlier_count`` are the ints ``n`` and ``k``,
    ``outlier_ratio`` is ``round(float(k / n), 6)``, ``max_score`` is
    ``round(float(max(score)), 6)`` and ``quality`` is ``"pass"`` only
    when ``k / n <= max_outlier_ratio``, otherwise ``"fail"``.
    ...
    if isinstance(max_outlier_ratio, float) and not math.isfinite(max_outlier_ratio):
    ...
    n = int(len(detected))
    k = int(sum(1 for is_outlier, _, _ in detected if is_outlier))
    outlier_ratio = rounded_float(k / n)
    max_score = rounded_float(max(score for _, score, _ in detected))
    document = {
        "results": [
