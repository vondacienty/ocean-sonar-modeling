...
def scale_gate_report(
    ...
    """Attach a flat margin summary to a :func:`scale_gate` result.
    ...
    called exactly once as
    ``scale_gate(fine, coarse, tolerances, min_within_ratio,
    max_rmse_limit)``; its exceptions propagate unchanged and inputs are
    not modified.

    With ``G`` the dict returned by that call and
    ``S = G["report"]["summary"]``, returns a dict with keys in the
    order ``report, summary, quality``: ``report`` is ``G["report"]``
    as-is and ``quality`` is ``G["quality"]``. ``summary`` has keys in
    the order ``tolerance_count, pass_count, fail_count, within_ratio,
    max_rmse, min_within_ratio, max_rmse_limit, within_margin,
    rmse_margin``: the first five values are copied as-is from ``S``
    (no recomputation, sorting or rewriting); ``min_within_ratio`` and
    ``max_rmse_limit`` are respectively
    ``round(float(min_within_ratio), 6)`` and
    ``round(float(max_rmse_limit), 6)``; ``within_margin`` is
    ``round(S["within_ratio"] - min_within_ratio, 6)`` and
    ``rmse_margin`` is
    ``round(max_rmse_limit - S["max_rmse"], 6)`` (negative zero
    normalized to ``0.0``). The counts are ints and every other value
    is a float.
    """
    G = scale_gate(
        fine, coarse, tolerances, min_within_ratio, max_rmse_limit
    )
