...
_AUDIT_REPORT_SUMMARY_KEYS = ("files", "changes", "failed", "passed", "pass_ratio")
...
def _dump_audit_report(report):
    """Re-serialize a validated audit report the way the writer would."""
    try:
        text = json.dumps(
            report,
            ensure_ascii=False,
...
def _check_audit_report(report):
    """Validate a decoded audit report document and return its normalized dict."""
    prefix = "audit report: "
    ...
    if list(summary.keys()) != list(_AUDIT_REPORT_SUMMARY_KEYS):
        raise TypeError(
            summary_prefix
            + "keys must be in the order files, changes, failed, passed, pass_ratio"
        )
    ...
    if pass_ratio != round(passed / changes, 6):
        raise ValueError(
    ...
    if list(worst.keys()) != list(_AUDIT_REPORT_WORST_KEYS):
        raise TypeError(
            worst_prefix
            + "keys must be in the order file_index, index, degraded_delta, "
            "coverage_delta, score_delta, quality"
        )
...
def load_audit_report(path) -> dict:
    ...
    duplicated, missing or extra keys are rejected. ``schema_version``
    must be the non-bool int ``1``. ``source`` must have keys exactly
    in the order ``path, kind`` with a non-empty ``str`` path and the
    fixed string ``"audit"``. ``summary`` must have keys exactly in the
    order ``files, changes, failed, passed, pass_ratio``: the first
    four must be non-bool ints with ``files >= 2``,
    ``changes >= files``, ``0 <= failed <= changes`` and
    ``passed == changes - failed``; ``pass_ratio`` must be a finite
    float equal to ``round(passed / changes, 6)`` with negative zero
    forbidden. ``worst`` must have keys exactly in the order
    ``file_index, index, degraded_delta, coverage_delta, score_delta,
    quality`` whose six items follow the ``worst`` contract of
    :func:`load_audit`: non-bool ints with ``0 <= file_index < files``,
    ``index >= 1`` and ``-2 <= degraded_delta <= 2``; finite non-bool
    floats in ``[-2, 2]`` and ``[-200, 200]`` respectively, each equal
    to ``round(float(v), 6)`` with negative zero forbidden; and a
    ``"pass"``/``"fail"`` string. The top-level ``quality`` must be
    ``"pass"`` if and only if ``failed`` is ``0``, in which case the
    ``worst`` quality must also be ``"pass"``. The file bytes must also
    equal the canonical re-serialization of the decoded value byte for
    byte; any key-order, type, range, relation, parse or canonical-byte
    mismatch raises ``ValueError``.

    Returns the audit report as a dict with the keys in the order
    above; every value is returned unchanged. The file is never
    modified.
    """
    if not isinstance(path, str):
        raise TypeError("path must be a str")
