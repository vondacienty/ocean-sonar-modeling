...
def load_trends(path) -> dict:
    ...
    The bytes must be exactly those produced by :func:`dump_trends`
    for the same value: compact UTF-8 JSON (``ensure_ascii=False``,
    ``separators=(",", ":")``, ``allow_nan=False``) with no BOM and no
    trailing newline. A BOM, a trailing newline, a UTF-8 decoding
    failure or a JSON parsing failure raises ``ValueError``; the
...
def compare_reports(paths) -> dict:
    """Compare successive :func:`load_trend_report` snapshots.
    ...
    Every loaded report must have a ``sources`` list equal, item by
    item in the same order, to that of the first report; otherwise a
    ``ValueError`` is raised.

    For each adjacent pair ``i = 1..n-1`` of report summaries, the
    unrounded deltas are
    ``dd = degraded_i - degraded_(i-1)``,
    ``dc = coverage_delta_i - coverage_delta_(i-1)`` and
    ``ds = score_delta_i - score_delta_(i-1)``. The comparison is a
    failure when ``dd > 0``, ``dc < 0``, ``ds < 0`` or the summary
    ``quality`` changes from ``"pass"`` to ``"fail"``, and a pass
    otherwise. Comparisons are not sorted, deduplicated or augmented.

    Returns a dict with keys in the order ``changes, worst, quality``.
    ``changes`` is a tuple with one dict per adjacent pair (in index
    order), each with keys in the order
    ``index, degraded_delta, coverage_delta, score_delta, quality``:
    ``index`` is the int ``i`` and ``degraded_delta`` is the int
    ``dd``; ``coverage_delta`` and ``score_delta`` are
    ``round(float(v), 6)`` floats (negative zero normalized to
    ``0.0``); and ``quality`` is ``"pass"`` or ``"fail"``. The worst
    selection uses the *unrounded* values: ``worst`` is the change dict
    whose tuple ``(ds, dc, -dd, i)`` is lexicographically smallest.
    The top-level ``quality`` is ``"pass"`` only when every change
    passes and ``"fail"`` otherwise.
    """
    if not isinstance(paths, (list, tuple)):
        raise TypeError("paths must be a list or tuple")
    if len(paths) < 2:
    ...
    for i in range(1, len(reports)):
        if reports[i]["sources"] != first_sources:
            raise ValueError(
                f"trend report at paths[{i}] sources "
                f"{reports[i]['sources']!r} do not match {first_sources!r}"
            )
    ...
    for i in range(1, len(reports)):
        ...
        rounded_dc = round(float(dc), 6)
        if rounded_dc == 0:
            rounded_dc = 0.0
        rounded_ds = round(float(ds), 6)
        if rounded_ds == 0:
            rounded_ds = 0.0
        ...
        change = {
            ...
            "coverage_delta": rounded_dc,
            "score_delta": rounded_ds,
            "quality": quality,
        }
...
def _check_comparison_change(change, expected_index, prefix):
    """Validate one comparison change object and return its normalized dict.

    ``expected_index`` is the required ``index`` value, or ``None`` to skip
    the index/consecutiveness check (used for ``worst``).
    """
    ...
    if expected_index is not None and index != expected_index:
        raise ValueError(
            prefix + f"index must be {expected_index} (consecutive from 1)"
        )

    degraded_delta = change["degraded_delta"]
    if type(degraded_delta) is not int:
        raise TypeError(prefix + "degraded_delta must be a non-bool int")
    if not -2 <= degraded_delta <= 2:
        raise ValueError(prefix + "degraded_delta must be in [-2, 2]")
...
def _check_comparison(comparison):
    prefix = "comparison: "
    if not isinstance(comparison, dict):
        raise TypeError("comparison must be a dict")
    if list(comparison.keys()) != list(_COMPARISON_KEYS):
        raise TypeError(
    ...
    for i in range(len(changes)):
        item = _check_comparison_change(
            changes[i], i + 1, f"{prefix}changes[{i}]: "
        )
        items.append(item)
    ...
    if list(worst.keys()) != list(_COMPARISON_CHANGE_KEYS):
        raise TypeError(
            worst_prefix
            + "keys must be in the order "
            "index, degraded_delta, coverage_delta, score_delta, quality"
    ...
    if matched is None:
        raise ValueError(
            worst_prefix
            + "must be field-for-field equal to one of the changes items"
        )
    ...
    return {"changes": tuple(items), "worst": matched, "quality": quality}


def _dump_comparison(comparison):
    document = {
        "changes": [dict(change) for change in comparison["changes"]],
        "worst": dict(comparison["worst"]),
        "quality": comparison["quality"],
    }
    try:
        text = json.dumps(
...
def load_comparison(path) -> dict:
    ...
    objects whose keys are exactly in the order
    ``index, degraded_delta, coverage_delta, score_delta, quality``:
    ``index`` and ``degraded_delta`` must be non-bool ints, with
    ``index`` running consecutively from ``1`` and
    ``degraded_delta`` in ``[-2, 2]``; ``coverage_delta`` and
    ``score_delta`` must be finite non-bool floats in ``[-2, 2]`` and
    ``[-200, 200]`` respectively, each equal to
    ``round(float(v), 6)`` with negative zero forbidden; and
    ``quality`` must be ``"pass"`` or ``"fail"``. ``worst`` must have
    the same key order and be field-for-field equal to one of the
    ``changes`` items. The top-level ``quality`` must be ``"pass"`` if
    and only if every change quality is ``"pass"``, and ``"fail"``
    otherwise. The file bytes must also equal the canonical
    re-serialization of the decoded value byte for byte; any
    key-order, type, range, consecutiveness, relation, parse or
    canonical-byte mismatch raises ``ValueError``.

    Returns the comparison as a dict with the keys in the order above;
    only the ``changes`` array is restored to a tuple (its ``worst``
    item is the matching tuple entry) and every other value is
    returned unchanged. The file is never modified.
    """
    if not isinstance(path, str):
        raise TypeError("path must be a str")
    ...
    try:
        result = _check_comparison(parsed)
...
def audit_comparisons(paths) -> dict:
    """Aggregate several :func:`serialize_comparison` JSON files.

    ``paths`` follows the exact input contract of :func:`compare_reports`:
    a list/tuple of at least two items; each item, checked in index order,
    ...
    :func:`load_comparison` is then called exactly once per path, in input
    order; any exception it raises is propagated unchanged. Inputs and the
    loaded files are not modified.

    The loaded comparisons are expanded in file order and, within each
    file, in the original ``changes`` order; comparisons are not sorted,
    deduplicated or augmented. With ``K`` the total number of change items
    and ``E`` the number whose ``quality`` is ``"fail"``, the worst item
    minimizes the tuple
    ``(score_delta, coverage_delta, -degraded_delta, file index, index)``
    lexicographically over the values as loaded.

    Returns a dict with keys in the order
    ``files, changes, failed, worst, quality``; ``files`` is the number of
    files, ``changes`` is ``K`` and ``failed`` is ``E``, all ints.
    ``worst`` is the six-item tuple
    ``(file index, index, degraded_delta, coverage_delta, score_delta,
    quality)`` whose first three items are ints, the next two floats and
    the last the item's ``"pass"``/``"fail"`` string, all copied as
    loaded. ``quality`` is ``"pass"`` only when ``E`` is ``0`` and
    ``"fail"`` otherwise.
    """
    if not isinstance(paths, (list, tuple)):
        raise TypeError("paths must be a list or tuple")
    ...
    for file_index in range(len(paths)):
        comparison = load_comparison(paths[file_index])
...
def export_audit(paths, output) -> bytes:
    """Serialize an audit of comparison files and atomically write it to disk.
    ...
    With ``B`` the ``bytes`` returned by :func:`serialize_audit`,
...
def serialize_audit_report(path) -> bytes:
    ...
    With ``A`` the dict returned by :func:`load_audit`, the encoded
    ...
    - ``source`` has keys in the order ``path, kind`` with the original
      ``path`` and the fixed string ``"audit"``;
    - ``summary`` has keys in the order ``files, changes, failed,
      passed, pass_ratio``: the first three are the ints ``A["files"]``,
      ``A["changes"]`` and ``A["failed"]``, ``passed`` is the int
      ``changes - failed`` and ``pass_ratio`` is
      ``round(float(passed / changes), 6)``;
    - ``worst`` has keys in the order ``file_index, index,
      degraded_delta, coverage_delta, score_delta, quality`` whose six
      values are the items of ``A["worst"]`` in their original order;
    - ``quality`` is ``A["quality"]``.

    All values are taken directly from ``A`` with no recomputation of
    the worst item, no mutation of ``A`` and no additional keys. The
    counts are ints; the two deltas and ``pass_ratio`` are floats.

    The object is encoded as UTF-8 JSON with ``ensure_ascii=False``,
    ``separators=(",", ":")``, ``allow_nan=False``, no indentation, no
    BOM and no trailing newline, exactly as in :func:`serialize_audit`.
    Floats are rounded at write time with ``round(float(v), 6)`` and
    ...
    document = {
        ...
        "summary": {
            ...
            "failed": failed,
            "passed": passed,
            "pass_ratio": rounded_float(passed / changes),
        },
        "worst": {
...
def _dump_audit_report_trend(report):
    """Serialize a ``sources, trend`` audit report trend like the exporter."""
    trend = report["trend"]
    worst = trend["worst"]
    document = {
        "sources": list(report["sources"]),
...
def export_audit_report_trend(paths, output) -> bytes:
    """Serialize an :func:`audit_report_trend` report and atomically write it.
    ...
    With ``T`` the dict returned by :func:`audit_report_trend`, ``output``
    is then validated: it must be a non-empty ``str`` (a non-str raises
    ``TypeError`` and an empty ``str`` raises ``ValueError``), in that
    order.

    The encoded object is compact UTF-8 JSON with top-level keys exactly
    in the order ``sources, trend``: ``sources`` is the ``paths`` items in
    their original order as a JSON array of strings, and ``trend`` is
    ``T`` itself, with its keys ``count, changes, regressed, failed_delta,
    pass_ratio_delta, worst, quality`` and values taken unchanged (its
    ``worst`` tuple encoded as the four-item JSON array). Encoding
    parameters (``ensure_ascii=False``, ``separators=(",", ":")``,
    ``allow_nan=False``), the absence of a BOM or trailing newline and the
    ``ValueError`` raised on any JSON or UTF-8 encoding failure all follow
    :func:`export_trends`.

    ``output`` must not name the same file as any of the ``paths`` items,
    and the write itself is atomic, both exactly as in
    :func:`export_audit`: existing files are compared with
    ``os.path.samefile`` so soft and hard links are recognized, otherwise
    the normalized paths
    ``os.path.normcase(os.path.realpath(os.path.abspath(path)))`` are
    compared and an overlap raises ``ValueError``; when there is no
    overlap a temporary file in ``output``'s directory receives the bytes,
    is ``flush()``ed and ``os.fsync()``ed and atomically replaces
    ``output`` via ``os.replace``. Any failure before the replacement
    removes the temporary file and leaves an existing ``output`` byte for
    byte unchanged; ``OSError`` is propagated unchanged.
...
def _check_audit_report_trend_trend(trend, source_count, prefix):
    """Validate a decoded ``trend`` against the :func:`audit_report_trend` contract."""
    if not isinstance(trend, dict):
        raise TypeError(prefix + "must be a dict")
    if list(trend.keys()) != list(_AUDIT_REPORT_TREND_TREND_KEYS):
        raise TypeError(
            prefix + "keys must be in the order count, changes, regressed, "
            "failed_delta, pass_ratio_delta, worst, quality"
        )
    ...
    _check_trend_int(
        count,
        "count",
        lambda v: v == source_count,
        f"{source_count} (the number of sources)",
        prefix,
    )
    ...
    _check_trend_int(
        changes,
        "changes",
        lambda v: v == source_count - 1,
        f"{source_count - 1} (count - 1)",
        prefix,
    )
    ...
    if type(failed_delta) is not int:
        raise TypeError(prefix + "failed_delta must be a non-bool int")

    _check_trend_float(
        trend["pass_ratio_delta"], "pass_ratio_delta", -1, 1, prefix
    )
    ...
    _check_trend_int(
        worst[0], "i", lambda v: 1 <= v < count, "in [1, count)", worst_prefix
    )
    ...
    if quality not in ("pass", "fail"):
        raise ValueError(prefix + "quality must be 'pass' or 'fail'")
    if (quality == "pass") != (trend["regressed"] == 0):
        raise ValueError(
...
def load_audit_report_trend(path) -> dict:
    ...
    ``path`` validation, binary (``"rb"``) reading and system exceptions
    (a missing file raises ``FileNotFoundError``, a directory raises
    ``IsADirectoryError`` and every other ``OSError`` is propagated
    unchanged), the BOM and trailing-newline rejection, UTF-8/JSON
    decoding, the rejection of ``NaN``/``Infinity`` and duplicate keys,
    and the canonical byte-for-byte re-serialization check all follow
    :func:`load_trend_report`; parse or decoding failures raise
    ...
    strings. ``trend`` must satisfy the :func:`audit_report_trend`
    return contract: keys exactly in the order ``count, changes,
    regressed, failed_delta, pass_ratio_delta, worst, quality`` with
    ``count`` equal to the number of ``sources`` and ``changes`` equal
    to ``count - 1``; ``count``/``changes``/``failed_delta`` non-bool
    ints, ``regressed`` a non-bool int in ``[0, changes]``,
    ``pass_ratio_delta`` a finite non-bool float in ``[-1, 1]`` equal to
    ``round(float(v), 6)`` with negative zero forbidden, ``worst`` the
    four-item array ``[i, df, dr, quality]`` with ``i`` a non-bool int
    in ``[1, count)``, ``df`` a non-bool int, ``dr`` a finite non-bool
    float in ``[-1, 1]`` with the same rounding and negative-zero rules
    and ``quality`` a ``"pass"``/``"fail"`` string, and the top-level
    ``quality`` ``"pass"`` if and only if ``regressed`` is ``0``;
    otherwise a ``ValueError`` is raised.

    :func:`audit_report_trend` is then called exactly once with
    ``sources`` to get ``E``; any exception it raises is propagated
    unchanged. The stored ``trend`` must equal ``E`` field by field
    (with ``worst`` compared as a tuple), otherwise a ``ValueError`` is
    raised.
    ...
    try:
        ...
        if list(parsed.keys()) != list(_AUDIT_REPORT_TREND_KEYS):
            raise TypeError(
                "audit report trend keys must be in the order sources, trend"
            )

        sources = parsed["sources"]
        sources_prefix = "audit report trend: sources: "
        if not isinstance(sources, list):
            raise TypeError(sources_prefix + "must be a list")
        if len(sources) < 2:
            raise ValueError(sources_prefix + "must contain at least 2 items")
        ...
        _check_audit_report_trend_trend(
            parsed["trend"], len(sources), "audit report trend: trend: "
        )
    except (TypeError, ValueError) as exc:
        raise ValueError(
    ...
    expected = audit_report_trend(list(sources))
    ...
    result = {
        ...
        "trend": {
            ...
            "failed_delta": trend["failed_delta"],
            "pass_ratio_delta": trend["pass_ratio_delta"],
            "worst": tuple(trend["worst"]),
            "quality": trend["quality"],
        },
    }
    if result["trend"] != expected:
        raise ValueError(
            "audit report trend: trend does not match "
            "audit_report_trend(sources)"
        )
