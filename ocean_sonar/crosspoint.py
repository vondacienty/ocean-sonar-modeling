...
def _check_audit(audit):
    ...
    if list(audit.keys()) != list(_AUDIT_KEYS):
        raise TypeError(
            "audit keys must be in the order files, changes, failed, "
            "worst, quality"
        )
    ...
    worst_prefix = prefix + "worst: "
    if not isinstance(worst, list):
        raise TypeError(worst_prefix + "must be an array")
    if len(worst) != 6:
        raise ValueError(worst_prefix + "must have 6 items")

    file_index = worst[0]
    if type(file_index) is not int:
        raise TypeError(worst_prefix + "file_index must be a non-bool int")
    if not 0 <= file_index < files:
        raise ValueError(worst_prefix + f"file_index must be in [0, {files})")
    ...
    _check_trend_float(
        worst[3], "coverage_delta", -2, 2, worst_prefix + "coverage_delta: "
    )
    _check_trend_float(
        worst[4], "score_delta", -200, 200, worst_prefix + "score_delta: "
    )
    ...
    if quality not in ("pass", "fail"):
        raise ValueError(prefix + "quality must be 'pass' or 'fail'")
    expected_quality = "pass" if failed == 0 else "fail"
    if quality != expected_quality:
        raise ValueError(
            prefix + "quality must be 'pass' if and only if failed is 0"
        )
    if quality == "pass" and worst_quality != "pass":
        raise ValueError(
            worst_prefix + "quality must be 'pass' when the audit quality "
            "is 'pass'"
        )
...
def _dump_audit(audit):
    worst = audit["worst"]
    document = {
        "files": int(audit["files"]),
        "changes": int(audit["changes"]),
...
def load_audit(path) -> dict:
    ...
    The bytes must be exactly those produced by :func:`serialize_audit`
    for the same value: compact UTF-8 JSON (``ensure_ascii=False``,
    ``separators=(",", ":")``, ``allow_nan=False``) with no BOM and no
    trailing newline. A BOM, a trailing newline, a UTF-8 decoding
    failure or a JSON parsing failure raises ``ValueError``; the
    ...
    ``files >= 2``, ``changes >= files`` and ``0 <= failed <=
    changes``. ``worst`` must be a six-item array ``[file_index, index,
    degraded_delta, coverage_delta, score_delta, quality]``: the first
    three items must be non-bool ints with ``0 <= file_index < files``,
    ``index >= 1`` and ``degraded_delta`` in ``[-2, 2]``;
    ``coverage_delta`` and ``score_delta`` must be finite non-bool
    floats in ``[-2, 2]`` and ``[-200, 200]`` respectively, each equal
    to ``round(float(v), 6)`` with negative zero forbidden; and the
    last item must be ``"pass"`` or ``"fail"``. The top-level
    ``quality`` must be ``"pass"`` if and only if ``failed`` is ``0``,
    and when it is ``"pass"`` the ``worst`` quality must be ``"pass"``
    as well. The file bytes must also equal the canonical
    re-serialization of the decoded value byte for byte; any key-order,
    type, range, relation, parse or canonical-byte mismatch raises
    ``ValueError``.

    Returns the audit as a dict with the keys in the order above; only
    the ``worst`` array is restored to a tuple and every other value is
    returned unchanged. The file is never modified.
    """
    if not isinstance(path, str):
    ...
    try:
        ...
        raise ValueError(
            f"file does not contain a valid audit: {exc}"
        ) from exc
