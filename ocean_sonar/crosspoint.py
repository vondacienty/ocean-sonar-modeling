...
def pair_gate_score(
    ...
    ``S = G["report"]["report"]["summary"]``, returns a dict with keys
    in the
    order ``pair_gate, matching, score, quality``. ``pair_gate`` and
    ``matching`` are ``G`` and ``M`` themselves. ``score`` is the float
    ``round(100 * M["first_coverage"] * M["second_coverage"]
    * S["mean_ratio"], 6)`` with negative zero normalized to ``0.0``.
    ``quality`` is ``"pass"`` only when ``R["quality"]`` is ``"pass"``
    and ``score`` equals ``100.0``, and ``"fail"`` otherwise.
    """
