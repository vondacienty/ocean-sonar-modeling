...
def export_audit_report(audit_path, output) -> bytes:
    ...
    Calls :func:`serialize_audit_report` exactly once with
    ``audit_path`` — before any other work and no second time — so its
    validation, first-error order, exceptions (propagated unchanged)
    and file invariants all apply here as well; in particular a bad
    ``audit_path`` value is reported before ``output`` is inspected.
    ``audit_path`` is not modified.
    ...
    _reject_export_output_overlap(output, [audit_path])
    _atomic_write_bytes(output, data)
    return data
