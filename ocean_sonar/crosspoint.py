...
def render_audit_report_trend(path) -> str:
    ...
    (propagated unchanged), file handling, source-report recheck and
    ...
    every other ``OSError`` is propagated unchanged and any parse, structure
    or canonical-byte violation raises ``ValueError``. The file is not
    modified.
    ...
    The TREND values are the six same-named items of ``T`` in its stored
    key order and the WORST values are the four items of ``W`` in their
    stored order. Every value is copied directly from ``R`` with no
    recomputation, re-sorting or rewriting: ints are formatted in decimal,
    strings are copied as-is and floats use ``format(v, ".6f")`` (negative
