...
def serialize_pair_gate_score_summary(
    ...
    """Encode :func:`pair_gate_score_summary` results as UTF-8 JSON bytes.
    ...
    quality``: the first four values are ``R["summary"]`` values
    copied as-is with no recomputation, sorting or added keys, and
    ``quality`` is ``R["quality"]``. ``coverage_product`` and
    ``score_margin`` are floats encoded as ``round(float(v), 6)`` with
    negative zero normalized to ``0.0``, ``matched`` is an int encoded
    in decimal, and ``pair_quality`` and ``quality`` are strings
    copied as-is.
