...
def pair_gate_report(
    ...
    (propagated unchanged) and index prefixes all apply here as well.
    Inputs and the :func:`pair_gate` result are not modified.

    With ``G`` the dict returned by :func:`pair_gate`, ``P = G["pairs"]``,
    ``m = len(P)``, ``f = len(first)`` and ``s = len(second)``, returns a
    dict with keys in the order ``pair_gate, matching, quality``.
    ``pair_gate`` is ``G`` itself. ``matching`` is a dict with keys in
    the order ``matched, first_total, second_total, first_coverage,
    second_coverage``; the first three are the ints ``m``, ``f`` and
    ``s``, and the last two are the floats ``round(m / f, 6)`` and
    ``round(m / s, 6)`` (negative zero normalized to ``0.0``).
    ``quality`` is ``"pass"`` only when ``G["quality"]`` is ``"pass"``
    and both coverages are ``1.0``, and ``"fail"`` otherwise.
    """
    result = pair_gate(
    ...
    first_coverage = round(float(m / f), 6)
    if first_coverage == 0:
        first_coverage = 0.0
    second_coverage = round(float(m / s), 6)
