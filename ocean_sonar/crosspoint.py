...
def pair_gate_quality(
    ...
    """Gate :func:`pair_gate_score_summary` results against quality thresholds.
    ...
    The thresholds are then validated in order: first ``min_coverage``,
    then ``min_score``. Each must be a finite non-bool int/float;
    ``min_coverage`` must lie in ``[0, 1]`` and ``min_score`` must lie
    in ``[0, 100]``. Type mismatches raise ``TypeError``; non-finite or
    ...
    order ``coverage_ok, score_ok``; they are the bools
    ...
    score_ok = bool(
        result["score_report"]["score_report"]["score"] >= min_score
    )
    ...
    quality = (
        "pass"
        if result["summary"]["pair_quality"] == "pass"
        and coverage_ok
        and score_ok
        else "fail"
    )
