...
def batch(observations, limit=1.0) -> bytes:
    """Correct a batch of observations and serialize the results as JSON bytes.
    ...
    non-bool int/float with ``limit >= 0``; finiteness is checked only
    for a float, so a non-negative int of any magnitude is valid. A bool
    or non-number raises ``TypeError`` and a non-finite float or a
    negative value raises ``ValueError``.

    With ``R`` the tuple returned by :func:`correct`, each observation
    ``i`` yields the adjustment
    ``a = round(float(R[i][2] - observations[i][2]), 6)`` (negative zero
    normalized to ``0.0``) and the flag ``w = abs(a) <= limit``.

    The returned document has top-level keys exactly in the order
    ``results, summary``: ``results`` is an array in observation order
    of ``[X, Y, D, a, w]`` arrays with ``X``/``Y``/``D`` taken from
    ``R[i]``; ``summary`` has keys exactly in the order ``count,
    pass_count, max_abs_adjustment, quality`` where ``count`` and
    ``pass_count`` are ints, ``max_abs_adjustment`` is
    ...
    document = {
        "results": [
            [
                rounded_float(corrected[i][0]),
                rounded_float(corrected[i][1]),
                rounded_float(corrected[i][2]),
                adjustments[i],
                within[i],
            ]
            for i in range(len(corrected))
