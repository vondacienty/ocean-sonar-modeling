...
def batch(times, depths, tide_times, levels, datum=0.0, limit=1.0) -> bytes:
    ...
    finiteness is checked only for a float, so a non-negative int of any
    magnitude is valid. A bool or non-number raises ``TypeError`` and a
    non-finite float or a negative value raises ``ValueError``.
    ...
    of ``[R[i], a, w]`` triples; ``summary`` has keys exactly in the
    order ``count, pass_count, max_abs_adjustment, quality`` where
    ``count`` and ``pass_count`` are ints, ``max_abs_adjustment`` is
