...
def dashboard_summary(
    ...
    :func:`~ocean_sonar.grid.build` (points, bounds, resolutions),
    then :func:`~ocean_sonar.terrain.analyze_layers`, then the
    :func:`~ocean_sonar.report.dashboard_summary` order (its internal
    ``crosspoint.dashboard``, ``assess`` and ``classify`` parameter
    checks, including their index prefixes). Inputs are neither
    modified nor reordered and no other function is called.

    Returns the dict returned by
    :func:`~ocean_sonar.report.dashboard_summary` unchanged (the same
    object, identity preserved), with keys in the order
    ``dashboard, summary``. All nested key orders, tuple levels, types,
    tolerance order, six-decimal rounding, negative-zero normalization
    and quality verdicts are exactly those produced by
    :func:`~ocean_sonar.report.dashboard_summary`.
    """
    grids = grid.build(points, bounds, resolutions)
    analyzed = terrain.analyze_layers(grids)
