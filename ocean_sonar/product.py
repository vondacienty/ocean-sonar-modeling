...
def write(product, path) -> bytes:
    ...
    serialized first — and :func:`serialize` is called exactly once,
    before ``path`` is touched — so every :func:`serialize` exception
    (``TypeError``/``ValueError``) is propagated unchanged and the file
    is not touched; only then is ``path`` validated — a non-str
