...
reads a batch JSON document back from a file.
"""
...
def _check_batch_summary(batch):
    if not isinstance(batch, dict):
        raise ValueError("batch summary must be a dict")
    ...
    if type(batch_count) is not int:
        raise ValueError("batch summary: batch_count must be a non-bool int")
    if not batch_count > 0:
        raise ValueError("batch summary: batch_count must be > 0")

    tolerance_count = batch["tolerance_count"]
    if type(tolerance_count) is not int:
        raise ValueError("batch summary: tolerance_count must be a non-bool int")
    if not tolerance_count > 0:
        raise ValueError("batch summary: tolerance_count must be > 0")

    pass_count = batch["pass_count"]
    if type(pass_count) is not int:
        raise ValueError("batch summary: pass_count must be a non-bool int")
    if not 0 <= pass_count <= tolerance_count:
        raise ValueError(
    ...
    if type(fail_count) is not int:
        raise ValueError("batch summary: fail_count must be a non-bool int")
    if fail_count != tolerance_count - pass_count:
        raise ValueError(
    ...
    if type(terrain_total) is not int:
        raise ValueError("batch summary: terrain_total must be a non-bool int")
    if not terrain_total > 0:
        raise ValueError("batch summary: terrain_total must be > 0")

    terrain_valid = batch["terrain_valid"]
    if type(terrain_valid) is not int:
        raise ValueError("batch summary: terrain_valid must be a non-bool int")
    if not 0 <= terrain_valid <= terrain_total:
        raise ValueError(
    ...
    if type(terrain_coverage) is not float:
        raise ValueError("batch summary: terrain_coverage must be a float")
    if not math.isfinite(terrain_coverage):
    ...
    if type(quality_score) is not float:
        raise ValueError("batch summary: quality_score must be a float")
    if not math.isfinite(quality_score):
    ...
    if quality_score != expected_score:
        raise ValueError(
            ...
            "(pass_count / tolerance_count) * (terrain_valid / "
            "terrain_total), 6)"
        )
...
def load_batch(path) -> dict:
    ...
    The bytes must be exactly those produced by :func:`serialize_batch`
    ...
    ``first_pass_summary``, ``tt`` for ``terrain_total`` and ``tv`` for
    ``terrain_valid``, each of ``bc, tc, pc, fc, tt, tv`` must be a
    non-bool int with ``bc > 0``, ``tc > 0``, ``0 <= pc <= tc``,
    ``fc == tc - pc``, ``tt > 0`` and ``0 <= tv <= tt``; ``fp`` must
    be ``None`` when ``pc == 0`` and otherwise a non-bool int in
    ``[0, bc)``; and ``terrain_coverage`` and ``quality_score`` must be
    finite non-bool floats equal respectively to ``round(tv / tt, 6)``
    and ``round(100 * (pc / tc) * (tv / tt), 6)``, with negative zero
