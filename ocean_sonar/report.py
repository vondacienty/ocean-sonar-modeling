...
:func:`compare_batch` validates two such batch summaries and compares
them by quality score (with terrain coverage as the tie-breaker).
"""
...
def _check_dashboard_summary(summary, prefix="dashboard summary: ", label=None):
    if label is None:
...
def aggregate_dashboard_summaries(summaries) -> dict:
    """Aggregate multiple :func:`dashboard_summary` ``summary`` dicts.

    ``summaries`` must be a non-empty ``list`` or ``tuple``; each item
    must be a ``"summary"`` dict as returned by
    :func:`dashboard_summary` and is validated with the same contract
    as :func:`render_dashboard_summary`, in input order, stopping at the
    first error. A non-list/tuple outer container raises ``TypeError``
    and an empty one raises ``ValueError``; per-item errors are prefixed
    with ``summaries[i]: `` (the container/key-order messages read
    ``summaries[i]: must be a dict`` / ``summaries[i]: keys must be in
    the order ...``). Item container, key-order and field-type errors raise
    ``TypeError``; emptiness, finiteness, range and relation errors
    raise ``ValueError``. The input is not modified.

    With ``K`` the number of items, ``M`` the sum of their
    ``tolerance_count`` values, ``P`` the sum of their ``pass_count``
    values, ``N`` the sum of their ``terrain_total`` values and ``V``
    the sum of their ``terrain_valid`` values, returns a dict with keys
    in the order ``batch_count, tolerance_count, pass_count,
    fail_count, first_pass_summary, terrain_total, terrain_valid,
    terrain_coverage, quality_score``; the values are respectively ``K``,
    ``M``, ``P``, ``M - P``, the index of the first item with
    ``pass_count > 0`` (or ``None``), ``N``, ``V``, ``round(V / N, 6)``
    and ``round(100 * (P / M) * (V / N), 6)``. Counts are non-bool ints,
    ``first_pass_summary`` is a non-bool int or ``None`` and the last
    two values are non-bool floats with negative zero normalized to
    ``0.0``.
    """
    ...
    for i, item in enumerate(summaries):
        item_prefix = f"summaries[{i}]: "
        _check_dashboard_summary(
            item, prefix=item_prefix, label=f"summaries[{i}]:"
        )
        tolerance_count += item["tolerance_count"]
        pass_count += item["pass_count"]
        if first_pass_summary is None and item["pass_count"] > 0:
            first_pass_summary = i
        terrain_total += item["terrain_total"]
        terrain_valid += item["terrain_valid"]
...
def serialize_batch(summaries) -> bytes:
    ...
    ``summaries`` — and no other combining function — so its
    validation, first-error order, exception messages and index
    prefixes all apply unchanged; every ``TypeError``/``ValueError``
    from that call is propagated unchanged and the input is neither
    modified nor reordered.

    The returned batch dict is encoded with its keys exactly in the
    order ``batch_count, tolerance_count, pass_count, fail_count,
    first_pass_summary, terrain_total, terrain_valid,
    terrain_coverage, quality_score``; ``first_pass_summary`` keeps
    its ``None`` value when no item has ``pass_count > 0``.

    The batch summary is encoded as UTF-8 JSON with
...
def _check_batch(batch, prefix="batch summary: ", label=None):
    ...
    if pass_count == 0:
        if first_pass_summary is not None:
            raise ValueError(
                prefix + "first_pass_summary must be None when "
                "pass_count is 0"
            )
    else:
        if type(first_pass_summary) is not int:
            raise TypeError(
                prefix + "first_pass_summary must be None or a "
                "non-bool int"
            )
        if not 0 <= first_pass_summary < batch_count:
            raise ValueError(
                prefix + "first_pass_summary must be in "
                "[0, batch_count)"
            )
    ...
    if quality_score != expected_score:
        raise ValueError(
            prefix + "quality_score must equal round(100 * "
...
def compare_batch(first, second) -> dict:
    """Compare two batch summaries by quality score.

    ``first`` and ``second`` must each be a batch-summary dict as
    returned by :func:`aggregate_dashboard_summaries` and accepted by
    :func:`load_batch`, with keys exactly in the order
    ``batch_count, tolerance_count, pass_count, fail_count,
    first_pass_summary, terrain_total, terrain_valid,
    terrain_coverage, quality_score``. They are validated in the order
    ``first`` then ``second``, each stopping at the first error; field
    errors are prefixed with ``first: `` / ``second: `` (the
    container/key-order messages read ``first must be a dict`` /
    ``first keys must be in the order ...``).

    Writing ``bc`` for ``batch_count``, ``tc`` for
    ``tolerance_count``, ``pc`` for ``pass_count``, ``fc`` for
    ``fail_count``, ``fp`` for ``first_pass_summary``, ``tt`` for
    ``terrain_total``, ``tv`` for ``terrain_valid``, ``cv`` for
    ``terrain_coverage`` and ``sc`` for ``quality_score``, the six
    count fields ``bc, tc, pc, fc, tt, tv`` must each be a non-bool
    int with ``bc > 0``, ``tc > 0``, ``0 <= pc <= tc``,
    ``fc == tc - pc``, ``tt > 0`` and ``0 <= tv <= tt``; ``fp`` must
    be ``None`` when ``pc == 0`` and otherwise a non-bool int in
    ``[0, bc)``; and ``cv`` and ``sc`` must be finite non-bool floats
    equal respectively to ``round(tv / tt, 6)`` and
    ``round(100 * (pc / tc) * (tv / tt), 6)``, with negative zero
    normalized to ``0.0``. Container, key-order and field-type errors
    raise ``TypeError``; range, relation and finiteness errors raise
    ``ValueError``. The inputs are not modified.

    Returns a dict with keys in the order
    ``first_score, second_score, delta, winner``: ``first_score`` and
    ``second_score`` are the respective (validated) ``quality_score``
    values; ``delta`` is ``round(second_score - first_score, 6)`` as a
    non-bool float with negative zero normalized to ``0.0``; and
    ``winner`` is decided from the original ``quality_score`` values —
    ``"first"`` when the first is larger, ``"second"`` when the second
    is larger — with equal scores broken by ``terrain_coverage``
    (``"first"``/``"second"`` for the larger coverage, ``"tie"`` when
    those are equal too).
    """
