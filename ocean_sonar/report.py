...
and encodes it as UTF-8 JSON bytes and :func:`load_ranking` reads
such a JSON document back from a file.
"""
...
def load_ranking(path) -> dict:
    ...
    The decoded value must satisfy the full :func:`serialize_ranking`
    contract — top-level key order ``ranking, score_spread``;
    ``ranking`` a non-empty array of ``[index, score, coverage]``
    triples with ``index`` a non-bool non-negative int whose values
    form exactly the set ``{0, 1, ..., n - 1}``, ``score`` a finite
    float in ``[0, 100]`` and ``coverage`` a finite float in
    ``[0, 1]``; the triples ordered by ``score`` descending, then
    ``coverage`` descending, then ``index`` ascending; and
    ``score_spread`` a finite float equal to
    ``round(max(score) - min(score), 6)`` with negative zero
    normalized to ``0.0`` — and the file bytes must equal the
    canonical re-serialization of the decoded value byte for byte;
    any key-order, type, range, index-set, ordering, relation or
    normalization-byte mismatch raises ``ValueError``.

    JSON arrays are recursively converted to tuples and floats are
    rounded with ``round(float(v), 6)`` with negative zero normalized
    to ``0.0``, mirroring :func:`serialize_ranking`.
    ...
    try:
        canonical = serialize_ranking(ranking)
    except (TypeError, ValueError) as exc:
        raise ValueError(
