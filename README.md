...
在 `pair_gate` 结果上补充配对覆盖率信息。

执行时**仅调用一次**
`pair_gate(first, second, tolerances, match_tolerance, min_mean_ratio,
max_rmse_limit)`，得到 `G`；因此全部校验（`pair` 阶段与 `gate` 阶段）
与异常传播行为同 `pair_gate` 完全一致，任何异常都原样向上传播。
输入参数与 `G` 均不被修改。

记 `P = G["pairs"]`，`m = len(P)`，`f = len(first)`，
`s = len(second)`。

**返回：** 键序为 `pair_gate, matching, quality` 的 `dict`：

- `pair_gate`：`pair_gate` 返回的原 `dict` `G`（同一对象，不做修改）；
