...
`max_outlier_ratio` 可省略），非法内容抛 `ValueError`。成功时 stdout
输出 `batch` 返回字节的 UTF-8 解码文本加一个换行，stderr 为空，退出
码 0；文件不存在、内容非法等错误时 stdout 为空，stderr 严格输出
